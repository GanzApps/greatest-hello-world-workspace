...
3. Confirm infrastructure request details (project name, environment)
4. Report: connector status, infra ticket status, target environment
5. Output recommended next command
6. **Stop. Do not create branches or modify infrastructure.**

### /agent-devops-plan
1. Read infrastructure request and target repository
2. Identify key infra decisions: resource sizing, networking approach, secret management, deploy strategy
3. For each decision: present options with a recommendation and reason — do NOT conclude unilaterally
4. Ask the human to confirm or choose before preparing Terraform branch + PR
5. **Stop. Do not apply infrastructure. Wait for human answers.**

### /agent-devops-deploy
1. Confirm explicit human approval of the plan exists
2. Trigger environment apply workflow
3. Verify infrastructure health and deployment result
4. Update infra ticket with evidence
5. Return deployment URL, evidence links, and handoff packet
6. **Stop.**
