...
### /agent-planner-init
1. Read `.agent-config.yml`, verify docs + tickets connectors are connected
2. Read delivery record from `artifacts/delivery-records/`
3. Confirm approved tech design URL exists in delivery record
4. Check tickets connector for existing tickets for this initiative
5. Report: connector status, delivery record phase, existing ticket count if any
6. Output recommended next command
7. **Stop. Do not create or modify tickets.**

### /agent-planner-plan
1. Read approved PRD and tech design
2. Identify key planning decisions: workstream grouping, ticket granularity, FE/BE parallelization strategy, priority order
...
4. Ask the human to confirm or choose before producing the slice
5. **Stop. Do not create tickets. Wait for human answers.**

### /agent-planner-slice
1. Read confirmed workstream plan
2. Break workstreams into specific ticket titles with domain, story slug, and dependency mapping
3. Output full ticket list (titles + domain + dependencies) for human review
4. **Stop. Do not create tickets in the ticket tool. Wait for human confirmation.**

### /agent-planner-tickets
1. Read confirmed ticket slice
2. Create all tickets in the configured ticket tool with full body sections
3. Run ticket quality gate on each ticket
4. Output the ticket board URL and a review prompt — do NOT mark done or output handoff yet:
   - List tickets created with status (ready / blocked)
   - Ask human to open the board and reply **approve** or **request changes: [what]**
5. On **approve**: mark phase done, output handoff packet
6. On **request changes**: apply changes, re-prompt review
7. **Stop. Wait for human review response.**
