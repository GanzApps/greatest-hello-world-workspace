...
- tickets declare a service name (e.g. `tghw_fe`) under `Target Service` — resolve the actual repo URL by fetching the System Catalog at `connectors.code.catalog_url` in `.agent-config.yml`
- if the catalog entry for the service has no repo URL yet, set the ticket to `Blocked` with note: "System Catalog: tghw_fe repo URL not set — complete DLV-001-1 first"
...
Ticket lifecycle:
- `Status` field: `Not started` → `In progress` → `In progress` → `Done`
- `Execution Status` field: `Ready` → `Ready` → `In Review` (PR open) → `Done` (PR merged)
- Claim ticket: set `Status = In progress`
- Open PR: set `Execution Status = In Review`, add evidence comment with PR URL
- PR merged: set `Status = Done`, set `Execution Status = Done`, add final evidence comment
- if execution fails and is recoverable: set `Execution Status = Blocked`, comment reason
- never set `Done` before PR is merged
...
### /agent-engineer-init
1. Read `.agent-config.yml`, verify docs + tickets + code connectors are connected
2. Read delivery record from `artifacts/delivery-records/`
3. List all `Ready` tickets from the ticket tool
4. Report: connector status, ready ticket count, first recommended ticket
5. Output recommended next command
6. **Stop. Do not claim or implement any ticket.**

### /agent-engineer-context
1. Fetch the specified ticket from the ticket tool
2. Read all referenced artifacts: PRD story, tech design module, DoD, API contract
3. Output full ticket context summary ready for implementation review
4. **Stop. Do not implement. Wait for human to confirm ticket selection.**
...
4. Open PR, update ticket with evidence comment, move to `Review`
5. Return PR URL and handoff packet
6. **Stop. Do not claim another ticket.**
